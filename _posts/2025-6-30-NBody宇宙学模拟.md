...
设一个粒子 $p$ 位于
$\bm{x}_{p}$，它被一个由8个网格点构成的立方单元所包围.
我们在之前计算出了这 $8$ 个角点上的引力向量
$\left(\mathrm{d}\bm{v}/\mathrm{d}a\right)_{i,j,k}$. 作用在粒子 $p$
上的引力 $\left(\mathrm{d}\bm{v}/\mathrm{d}a\right)_{p}$ 就是这 $8$
个网格点上引力的加权平均：

$$\left(\frac{\mathrm{d}\bm{v}}{\mathrm{d}a}\right)_{p}=\sum_{l,m,n\in\{0,1\}}\left(\frac{\mathrm{d}\bm{v}}{\mathrm{d}a}\right)_{i+l,j+m,k+n}\cdot w_{i+l,j+m,k+n}$$
